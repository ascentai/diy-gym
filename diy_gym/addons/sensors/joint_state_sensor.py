...
class JointStateSensor(Addon):
    ...
    def __init__(self, parent, config):
        ...
        if 'joints' in config:
            self.joint_ids = [[p.getJointInfo(self.uid, i)[1].decode('utf-8') for i in range(p.getNumJoints(self.uid))].index(joint) for joint in config.get('joints')]
        else:
        ...
        self.observation_space = spaces.Dict({'position': spaces.Box(low=joint_position_lower_limit, high=joint_position_upper_limit, dtype='float32')})

        if self.include_velocity:
            joint_velocity_limit = np.array([info[11] for info in joint_info])
            self.observation_space.spaces['velocity'] = spaces.Box(low=-joint_velocity_limit, high=joint_velocity_limit, dtype='float32')
