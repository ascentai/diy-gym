...
class JointController(Addon):
    ...
    def __init__(self, parent, config):
        ...
        self.control_mode = {'position': p.POSITION_CONTROL, 'velocity': p.VELOCITY_CONTROL, 'torque': p.TORQUE_CONTROL}[config.get('control_mode', 'velocity')]
    ...
    def update(self, action):
        ...
        p.setJointMotorControlArray(self.uid, self.joint_ids, self.control_mode, positionGains=[0.03] * len(action), velocityGains=[1.0] * len(action), **kwargs)
