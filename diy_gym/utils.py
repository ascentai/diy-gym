...
def get_bounds_for_space(space, low_not_high):
    if isinstance(space, spaces.Discrete):
        ...
        return OrderedDict(
            sorted({key: get_bounds_for_space(subspace, low_not_high)
                    for key, subspace in space.spaces.items()}.items(),
                   key=lambda t: t[0]))
        ...
        try:
            ...
            raise AttributeError(
                "Could not find lower bound for space, did you define your own gym.Space? If so you'll need to define its upper and lower bound so that we can flatten it properly"
            )
...
def unflatten(to_unflatten, action_space):
    def _unflatten(extractor, space):
        if isinstance(space, spaces.Dict):
            return OrderedDict(
                sorted({key: _unflatten(extractor, subspace)
                        for key, subspace in space.spaces.items()}.items(),
                       key=lambda t: t[0]))
        elif isinstance(space, spaces.Tuple):
            ...
            raise AttributeError(
                "Unrecognised datatype in action_space; spaces other than those defined by the gym aren't currently supported by flatten_actions"
            )
