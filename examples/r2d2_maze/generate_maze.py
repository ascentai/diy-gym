...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_gripper_camera', type=lambda x:bool(strtobool(x)), default=False, help='whether to attach a camera to the R2D2s gripper')
    ...
    env_config = {
        ...
        'r2d2': {
            ...
            'wheel_driver': {
                ...
                'joints': ['left_front_wheel_joint', 'left_back_wheel_joint', 'right_front_wheel_joint', 'right_back_wheel_joint'],
                'control_mode': 'velocity'
            },
            'respawn': { 'addon': 'respawn' }
        },
        'plane': {
    ...
    while len(path):
        unvisited_neighbours = [tuple(path[-1] + offset) for offset in offsets if not tuple((path[-1] + offset).clip(0,args.maze_size-1)) in maze]
